...
- '피할 전개'는 장기 방향과 이번 흐름보다 우선한다.`;
  ...
  function resetReferenceCache(room = "") {
    referenceCache = {
      room,
      memoryAt: 0,
      memories: [],
  ...
  const ROOM_DRAFT_BACKUP_PREFIX = "cmwRoomDraftBackup_";
  let lastStableChatRoomId = "";
  let roomScopedUiLoadedFor = "";
  let roomDraftDirty = false;

  function detectChatRoomId(pathname = location.pathname) {
  ...
  function getChatRoomId() {
    const detected = detectChatRoomId();
    if (detected) {
      lastStableChatRoomId = detected;
      try { sessionStorage.setItem("cmwLastStableChatRoomId", detected); } catch (_) {}
      return detected;
    }
    if (lastStableChatRoomId) return lastStableChatRoomId;
    try {
      const remembered = String(sessionStorage.getItem("cmwLastStableChatRoomId") || "").trim();
      if (remembered) {
        lastStableChatRoomId = remembered;
        return remembered;
      }
    } catch (_) {}
    return "global_room";
  ...
  // 문장 뒤에 붙이는 언어 태그 감지용 — [en]·[zh-tw] 같은 코드,
  // [Spanish] 같은 영문 이름, [스페인어]·[중국어 간체] 같은 한글 이름을 모두 허용
  const LANG_TAG_RE = /\[(?:[a-z]{2,3}(?:-[A-Za-z]{2,4})?|[A-Z][a-z]{3,24}(?: [A-Z][a-z]{2,20})*|[가-힣]{1,8}어(?: ?(?:간체|번체))?)\](?=\s|$|["'”’)}])/;

  function extractLanguageTags(text) {
    return String(text || "").match(new RegExp(LANG_TAG_RE.source, "g")) || [];
  }

  function hasSameLanguageTags(before, after) {
    const expected = extractLanguageTags(before);
    const actual = extractLanguageTags(after);
    return expected.length === actual.length && expected.every((tag, index) => tag === actual[index]);
  }

  // LLM 출력·contenteditable 왕복 과정에서 불어나는 빈 줄을 최대 1줄로 압축
  ...
  function readGmRoomDraft(room) {
    return {
      pcNote: String(GM_getValue("cfgPcNote_" + room, "") || ""),
      customRule: String(GM_getValue("cfgCustomRule_" + room, "") || ""),
      lores: Array.from({ length: 10 }, (_, offset) => {
        const index = offset + 1;
        return {
          active: GM_getValue(getLoreActiveKey(room, index), false) === true,
          text: String(GM_getValue(getLoreTextKey(room, index), "") || ""),
        };
      }),
      savedAt: Number(GM_getValue(`cmwRoomDraftSavedAt_${room}`, 0)) || 0,
    };
  }

  function readRoomDraftBackup(room) {
    if (!room || room === "global_room") return null;
    try {
      const parsed = JSON.parse(localStorage.getItem(ROOM_DRAFT_BACKUP_PREFIX + room) || "null");
      if (!parsed || typeof parsed !== "object") return null;
      if (!Array.isArray(parsed.lores)) parsed.lores = [];
      return parsed;
    } catch (_) {
      return null;
    }
  ...
  function writeRoomDraftBackup(room, draft) {
    if (!room || room === "global_room" || !draft) return;
    try {
      localStorage.setItem(ROOM_DRAFT_BACKUP_PREFIX + room, JSON.stringify(draft));
    } catch (_) {}
  }

  function roomDraftHasContent(draft) {
    if (!draft) return false;
    return !!(
      String(draft.pcNote || "").trim() ||
      String(draft.customRule || "").trim() ||
      (draft.lores || []).some((item) => String(item?.text || "").trim())
    );
  }

  function writeGmRoomDraft(room, draft) {
    if (!room || room === "global_room" || !draft) return;
    GM_setValue("cfgPcNote_" + room, String(draft.pcNote || ""));
    GM_setValue("cfgCustomRule_" + room, String(draft.customRule || ""));
    for (let i = 1; i <= 10; i++) {
      const item = draft.lores?.[i - 1] || {};
      GM_setValue(getLoreActiveKey(room, i), item.active === true);
      GM_setValue(getLoreTextKey(room, i), String(item.text || ""));
    }
    const savedAt = Number(draft.savedAt) || Date.now();
    GM_setValue(`cmwRoomDraftSavedAt_${room}`, savedAt);
  }

  function migrateAccidentalGlobalRoomDraft(room) {
    if (!room || room === "global_room") return;
    const target = readGmRoomDraft(room);
    if (roomDraftHasContent(target)) return;
    if (readRoomDraftBackup(room)) return;
    const accidental = readGmRoomDraft("global_room");
    if (!roomDraftHasContent(accidental)) return;
    accidental.savedAt = Date.now();
    writeGmRoomDraft(room, accidental);
    writeRoomDraftBackup(room, accidental);
    GM_deleteValue("cfgPcNote_global_room");
    GM_deleteValue("cfgCustomRule_global_room");
    for (let i = 1; i <= 10; i++) {
      GM_deleteValue(getLoreActiveKey("global_room", i));
      GM_deleteValue(getLoreTextKey("global_room", i));
    }
    GM_deleteValue("cmwRoomDraftSavedAt_global_room");
  }

  function getReliableRoomDraft(room) {
    migrateAccidentalGlobalRoomDraft(room);
    const stored = readGmRoomDraft(room);
    const backup = readRoomDraftBackup(room);
    if (!backup) return stored;
    if (!roomDraftHasContent(stored) && roomDraftHasContent(backup)) {
      const restored = { ...backup, savedAt: Date.now() };
      writeGmRoomDraft(room, restored);
      writeRoomDraftBackup(room, restored);
      return restored;
    }
    if ((Number(backup.savedAt) || 0) > (Number(stored.savedAt) || 0)) {
      writeGmRoomDraft(room, backup);
      return backup;
    }
    return stored;
  }

  function collectRoomDraftFromUI(room = getChatRoomId()) {
    if (!room || room === "global_room" || roomScopedUiLoadedFor !== room) return null;
    return {
      pcNote: document.getElementById("cfg-pc-note")?.value || "",
      customRule: document.getElementById("cfg-custom-rule")?.value || "",
      lores: Array.from({ length: 10 }, (_, offset) => {
        const index = offset + 1;
        return {
          active: !!document.getElementById(`lore-active-${index}`)?.checked,
          text: document.getElementById(`lore-text-${index}`)?.value || "",
        };
      }),
      savedAt: Date.now(),
    };
  ...
  function persistRoomDraftFromUI(room = getChatRoomId()) {
    const draft = collectRoomDraftFromUI(room);
    if (!draft) return false;
    writeGmRoomDraft(room, draft);
    writeRoomDraftBackup(room, draft);
    roomDraftDirty = false;
    return true;
  }

  function backupRoomDraftFromUI(room = getChatRoomId()) {
    const draft = collectRoomDraftFromUI(room);
    if (!draft) return;
    roomDraftDirty = true;
    GM_setValue(`cmwRoomDraftSavedAt_${room}`, draft.savedAt);
    writeRoomDraftBackup(room, draft);
  }
  ...
  async function fetchAllLongTermMemories(force = false) {
    ...
    if (getChatRoomId() !== room) return all;
  ...
  async function readActiveEriLore(force = false) {
    ...
    if (getChatRoomId() !== room) {
      return { entries: [], packs: [], status: "방 이동으로 로어 조회 취소" };
    }
    const w = typeof unsafeWindow !== "undefined" ? unsafeWindow : window;
    const L = w.__LoreInj;
  ...
  GM_addStyle(`
        ...
        .crack-pure-trans.hold { transform:scale(1.12); box-shadow:0 0 0 3px rgba(84,164,240,.45), 0 4px 14px rgba(46,134,222,.5); }
        @keyframes crack-spin { to { transform:rotate(360deg); } }
        .crack-trans-lang-menu { position:fixed; z-index:2147483646; display:flex; flex-direction:column; width:176px; max-height:290px; overflow-y:auto; overscroll-behavior:contain; background:var(--bg_elevated_primary,#1d2027); border:1px solid var(--border,#3a3f4a); border-radius:12px; padding:6px; box-shadow:0 10px 28px rgba(0,0,0,.38); font-family:"CMW Pretendard","Pretendard Variable",sans-serif; }
        .crack-trans-lang-menu .trans-lang-menu-head { font-size:11px; font-weight:700; color:var(--text_secondary,#9aa3b2); padding:4px 8px 6px; }
        .crack-trans-lang-menu .trans-lang-item { display:block; width:100%; text-align:left; border:none; background:transparent; color:var(--text_primary,#e8eaf0); font-size:12.5px; padding:6px 8px; border-radius:8px; cursor:pointer; }
        .crack-trans-lang-menu .trans-lang-item:hover { background:rgba(84,164,240,.16); }
        .crack-trans-lang-menu .trans-lang-item.on { background:rgba(84,164,240,.24); color:#7cc0ff; font-weight:700; }
            ...
            <div class="panel-content">
                ...
                <div class="setting-group">
                    ...
                    <div style="font-size:11px; color:var(--text_secondary); line-height:1.55;">
                        태그가 하나도 없으면 위에서 선택한 목표 언어로 대사 전체를 번역합니다. 문장 뒤에 <b>[es]</b>, <b>[영어]</b>, <b>[Spanish]</b> 같은 태그가 하나라도 있으면 태그가 붙은 문장만 해당 언어로 번역하고, 태그 없는 문장은 한국어로 둡니다.<br>
                        예: 안녕, 반가워! 잘 지냈어?<b>[스페인어]</b> → 앞 문장은 그대로, 뒤 문장만 스페인어로 번역<br>
                        언어가 아닌 대괄호 표현은 일반 텍스트로 취급됩니다. 태그를 쓰는 동안 위 목표 언어 설정은 사용되지 않습니다.
                    </div>
                </div>
  ...
  async function refreshCurrentProfileFromApi(force = false) {
    ...
    if (profileScanInFlight && profileScanInFlightRoom === room) return profileScanInFlight;
    ...
    profileScanInFlight = requestPromise;
    profileScanInFlightRoom = room;
    const clearInFlight = () => {
      if (profileScanInFlight === requestPromise) {
        profileScanInFlight = null;
        profileScanInFlightRoom = "";
      }
    };
  ...
  document.getElementById("cfg-pc-note").addEventListener("input", (e) => {
    const room = getChatRoomId();
    if (room === "global_room" || roomScopedUiLoadedFor !== room) return;
    GM_setValue("cfgPcNote_" + room, e.target.value);
    backupRoomDraftFromUI(room);
  });
  ...
  function cmwGotoPane(paneId) {
    ...
    if (paneId === "pane-compass") scrollAdvisorChatToBottom();
  ...
  async function refreshReferenceData(force = false, triggerTokenPreview = true) {
    const requestRoom = getChatRoomId();
    ...
    if (getChatRoomId() !== requestRoom) return;
  ...
  function scrollAdvisorChatToBottom() {
    ...
    requestAnimationFrame(() => {
      ...
      // 패널 표시·레이아웃 계산이 한 프레임 늦는 환경에서도 마지막 대화가 보이게 한다.
  ...
  const loadCfg = () => {
    ...
    const roomDraft = getReliableRoomDraft(room);
    ...
    document.getElementById("cfg-pc-note").value = roomDraft.pcNote || "";
    document.getElementById("cfg-custom-rule").value = roomDraft.customRule || "";
    ...
    for (let i = 1; i <= 10; i++) {
      const lore = roomDraft.lores?.[i - 1] || {};
      document.getElementById(`lore-active-${i}`).checked = lore.active === true;
      document.getElementById(`lore-text-${i}`).value = String(lore.text || "");
    }
    refreshLoreDictionaryUI();
    roomScopedUiLoadedFor = room;
    persistRoomDraftFromUI(room);
  ...
  function initPanelEvents() {
    ...
    for (let i = 1; i <= 10; i++) {
      ...
      active?.addEventListener("change", () => {
        const room = getChatRoomId();
        if (room === "global_room" || roomScopedUiLoadedFor !== room) return;
        GM_setValue(getLoreActiveKey(room, i), !!active.checked);
        backupRoomDraftFromUI(room);
        scheduleReferenceTokenPreview();
      });
      document.querySelector(`[data-lore-remove="${i}"]`)?.addEventListener("click", () => {
        ...
        const room = getChatRoomId();
        if (room === "global_room" || roomScopedUiLoadedFor !== room) return;
        GM_setValue(getLoreTextKey(room, i), "");
        GM_setValue(getLoreActiveKey(room, i), false);
        backupRoomDraftFromUI(room);
        refreshLoreDictionaryUI();
        scheduleReferenceTokenPreview();
      });
...
${conversation}`;
  ...
  function buildTranslateSysPrompt(sourceText = "") {
    const room = getChatRoomId();
    const lang = getTargetLang();
    const { pattern, includesOriginal } = buildTransFormatInstruction();
    const note = (GM_getValue("transNote_" + room, "") || "").trim();
    const taggedMode = LANG_TAG_RE.test(String(sourceText || ""));

    if (!taggedMode) {
      let singlePrompt = `You are a roleplay dialogue translator. Translate the user's roleplay text to ${lang}.
Rules:
...
3. CRITICAL: Output each dialogue segment EXACTLY in this format: ${pattern}`;
      ...
      singlePrompt += `
4. Preserve the input's line breaks and overall structure. NEVER insert blank lines or line breaks that are not present in the input, and never add markdown paragraph spacing.
Example Input: *손을 흔들며* 안녕, 반가워!
Example Output Structure: *손을 흔들며* ${pattern}
The <TRANSLATED_DIALOGUE> placeholder must contain ${lang}, not English unless the selected target language is English.`;
    ...
    const fmt = getTransFormatTemplate();
    const taggedExample = fmt
      .split("{번역문}").join("¿Has estado bien?")
      .split("{원문}").join("잘 지냈어?");
    const taggedEnglishExample = fmt
      .split("{번역문}").join("See you tomorrow.")
      .split("{원문}").join("내일 보자.");

    let sysPrompt = `You are a roleplay dialogue translator. The user's text is a Korean roleplay message. Translate ONLY the sentences explicitly marked with a language-code tag; everything else passes through unchanged.
Rules:
1. Any narration/action/inner-monologue wrapped in asterisks (*...* or **...**) must remain in the original Korean, EXACTLY as written. Do NOT translate or alter it.
2. LANGUAGE TAGS: A language tag is square brackets attached to the END of a sentence, containing either a short language code — e.g. [en], [es], [ja], [zh-tw] — or a language name in English or Korean — e.g. [Spanish], [스페인어], [일본어]. Interpret codes flexibly: [en]=English, [es]=Spanish, [ja] or [jp]=Japanese, [zh] or [cn]=Chinese, [ru]=Russian, etc.
3. SENTENCE SCOPE: Each tag applies ONLY to the single spoken-dialogue sentence immediately before that tag. Never cross an earlier sentence-ending mark (. ! ? …), a line break, a previous language tag, or the end of an asterisk-wrapped narration block. If the tag follows a quoted sentence, translate only that quoted sentence. Process multiple tags independently, in their original order. The tag itself must NEVER appear anywhere in the output, including the original-Korean part of the format.
4. NO TAG, NO TRANSLATION: Any sentence without its own trailing language tag must remain in the original Korean, EXACTLY as written. Never translate untagged dialogue, even partially. A [ko], [kr] or [한국어] tag means the sentence stays Korean — just remove the tag. Square brackets whose content is not a language code or language name are normal text, NOT tags; leave them untouched.
5. CRITICAL: Output each TRANSLATED sentence EXACTLY in this format: ${pattern}`;
    ...
    sysPrompt += `
6. Preserve the input's line breaks and overall structure. NEVER insert blank lines or line breaks that are not present in the input, and never add markdown paragraph spacing.
Example Input: *손을 흔들며* 안녕, 반가워! 잘 지냈어?[es] 내일 보자.[en]
Example Output: *손을 흔들며* 안녕, 반가워! ${taggedExample} ${taggedEnglishExample}`;

    if (note) {
      sysPrompt += `\n7. Apply this persona/speaking style to the translated dialogue: ${note}`;
    }
  ...
  function callTranslate(sourceText) {
    return requestTranslationLLM(buildTranslateSysPrompt(sourceText), sourceText, { temperature: 0.3 });
  }
  ...
  function callGemini(baseText, options = {}) {
    return new Promise(async (resolve, reject) => {
      ...
      const provider = options.provider || GM_getValue("apiProvider", "google");
      const room = getChatRoomId();
      const sourceLanguageTags = options.preserveLanguageTags ? extractLanguageTags(baseText) : [];
      ...
      const model = normalizeModelId(options.model || GM_getValue("cfgModel", "gemini-3.1-pro-preview"));
      const profileInfo = await refreshCurrentProfileFromApi(!options.preflightOnly).catch(() => {
        scanProfileFromDomFallback();
        return readStoredProfile(room);
      });
      ...
      const storedTones = readJsonValue("cfgTones", []);
      const toneList = Array.isArray(storedTones) ? storedTones : [];
...
- 결과는 사용자가 그대로 채팅 입력창에 넣을 수 있는 롤플레잉 본문이어야 한다.`);
      ...
      if (sourceLanguageTags.length) {
        sysPromptParts.push(`[번역 제어용 언어 태그 보존 — 필수]
현재 입력에는 문장별 번역 단계를 위한 제어 태그가 있다: ${JSON.stringify(sourceLanguageTags)}
- 각 태그는 바로 앞의 사용자 대사 문장에 속한다.
- 태그의 철자·대소문자·순서·개수를 절대 바꾸지 않는다.
- 태그를 번역·삭제·추가하거나 다른 문장으로 옮기지 않는다.
- 태그가 붙은 대사를 다듬거나 확장하더라도 해당 태그는 그 대사 문장의 맨 끝에 그대로 남긴다.
- 태그 하나가 맡는 번역 범위가 달라지지 않도록, 태그가 붙은 대사 한 문장을 여러 대사 문장으로 쪼개지 않는다.
- 이 단계에서는 태그가 가리키는 외국어로 직접 번역하지 않는다. 한국어 집필 결과와 원래 태그를 함께 출력하여 다음 번역 단계로 넘긴다.`);
      }
...
- Muse가 표식을 검증해 숨김 주석으로 바꾸므로 본문에서는 후크와 표식을 언급하지 않는다.`);
  ...
  function setChatInputText(text) {
    ...
    if (chatInput.tagName === "TEXTAREA") {
      ...
      if (setter) setter.call(chatInput, text);
  ...
  function updateChatInputFromHistory() {
    ...
    setChatInputText(generatedHistory[historyIndex] || "");
  ...
  function buildWrapperContents(wrapper) {
    ...
    // 2) 유저 입력 번역 버튼 (짧게: 번역 실행 / 550ms 길게: 목표 언어 변경)
    const tBtn = document.createElement("button");
    tBtn.id = "crack-pure-trans-btn";
    tBtn.type = "button";
    tBtn.className = "crack-pure-trans";
    tBtn.setAttribute("aria-label", "유저 입력 번역 — 짧게: 번역, 길게: 언어 변경");
    tBtn.innerHTML = `<span class="trans-glyph" aria-hidden="true">🌐</span>`;

    const refreshTransTitle = () => {
      tBtn.title = `유저 입력 번역 · 목표 언어: ${getTargetLang()} (길게 누르면 언어 변경)`;
    };
    refreshTransTitle();

    const closeTransLangMenu = () => {
      document.getElementById("crack-trans-lang-menu")?.remove();
    };

    const applyTransLang = (value) => {
      GM_setValue("cfgTransLang", value);
      const langSel = document.getElementById("cfg-trans-lang");
      if (langSel) {
        if (![...langSel.options].some((option) => option.value === value)) {
          const option = document.createElement("option");
          option.value = value;
    ...
    const openTransLangMenu = () => {
      closeTransLangMenu();
      const saved = GM_getValue("cfgTransLang", "English");
      const customLang = (GM_getValue("cfgTransCustomLang", "") || "").trim();
      const entries = TRANS_LANGUAGES
        .filter(([value]) => value !== "__custom__")
        .map(([value, label]) => ({ value, label }));
      if (customLang) entries.push({ value: "__custom__", label: `${customLang} (직접 입력)` });
      if (saved !== "__custom__" && !entries.some((entry) => entry.value === saved)) {
        entries.push({ value: saved, label: saved });
      }

      const menu = document.createElement("div");
      menu.id = "crack-trans-lang-menu";
      menu.className = "crack-trans-lang-menu";

      const head = document.createElement("div");
      head.className = "trans-lang-menu-head";
      head.textContent = "번역 목표 언어";
      menu.appendChild(head);

      entries.forEach(({ value, label }) => {
        const item = document.createElement("button");
        item.type = "button";
        item.className = "trans-lang-item" + (value === saved ? " on" : "");
        item.textContent = label;
        item.addEventListener("click", (event) => {
          event.preventDefault();
          event.stopPropagation();
          applyTransLang(value);
          closeTransLangMenu();
        });
      ...
      menu.style.left = `${Math.max(8, Math.min(rect.left + rect.width / 2 - menu.offsetWidth / 2, window.innerWidth - menu.offsetWidth - 8))}px`;
      menu.style.top = `${Math.max(8, rect.top - menu.offsetHeight - 10)}px`;
      menu.querySelector(".trans-lang-item.on")?.scrollIntoView({ block: "center" });
    ...
    const runTranslate = async () => {
      ...
      try {
        ...
        if (mode === "write") {
          setGlyph("✎", false);
          sourceText = collapseExtraBlankLines(await callGemini(baseText, {
    ...
    const TRANS_HOLD_MS = 550;
    const TRANS_RING_DELAY_MS = 300;
    let tHoldTimer = 0;
    let tRingTimer = 0;
    let tHoldFired = false;
    let tPressing = false;
    let tPointerId = null;
    let tStartX = 0;
    let tStartY = 0;
    let tMenuOpenAtPress = false;

    const clearTransPress = (pointerId = tPointerId) => {
      clearTimeout(tHoldTimer);
      clearTimeout(tRingTimer);
      tBtn.classList.remove("hold");
      tPressing = false;
      if (pointerId != null) {
        try {
          if (tBtn.hasPointerCapture?.(pointerId)) tBtn.releasePointerCapture(pointerId);
        } catch (_) {}
      }
      tPointerId = null;
    };
    ...
    tBtn.addEventListener("pointerdown", (e) => {
      ...
      e.preventDefault();
      clearTransPress();
      tMenuOpenAtPress = !!document.getElementById("crack-trans-lang-menu");
      closeTransLangMenu();
      tPressing = true;
      tHoldFired = false;
      ...
      tRingTimer = setTimeout(() => {
        if (tPressing && !tHoldFired) tBtn.classList.add("hold");
      }, TRANS_RING_DELAY_MS);
      tHoldTimer = setTimeout(() => {
        tHoldFired = true;
        clearTransPress(e.pointerId);
        openTransLangMenu();
      }, TRANS_HOLD_MS);
    });

    tBtn.addEventListener("pointermove", (e) => {
      if (!tPressing || e.pointerId !== tPointerId) return;
      if (Math.hypot(e.clientX - tStartX, e.clientY - tStartY) > 14) clearTransPress(e.pointerId);
    });
    tBtn.addEventListener("pointercancel", (e) => clearTransPress(e.pointerId));

    tBtn.addEventListener("pointerup", (e) => {
      if (tPointerId != null && e.pointerId !== tPointerId) return;
      const shouldTranslate = tPressing && !tHoldFired && !tMenuOpenAtPress;
      clearTransPress(e.pointerId);
    ...
    gBtn.addEventListener("pointerdown", (e) => {
      ...
      holdTimer = setTimeout(() => {
        ...
        if (willOpen) scrollAdvisorChatToBottom();
      }, HOLD_MS);
    });
  ...
  function injectUI() {
    ...
    if (currentRoomId !== newRoomId) {
      // 5.3.3의 방별 초안 보존을 먼저 끝낸 뒤, 4.5.1의 방 이동 초기화를 수행한다.
      if (currentRoomId && currentRoomId !== "global_room" && roomDraftDirty) persistRoomDraftFromUI(currentRoomId);
      resetHistory();
      resetReferenceCache(newRoomId);
  ...
  window.addEventListener("pagehide", () => { if (roomDraftDirty) persistRoomDraftFromUI(); });
  window.addEventListener("beforeunload", () => { if (roomDraftDirty) persistRoomDraftFromUI(); });
